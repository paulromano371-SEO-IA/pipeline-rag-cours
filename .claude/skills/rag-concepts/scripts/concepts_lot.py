"""Lance UN lot de /rag-concepts et termine TOUJOURS par une ligne de bilan.

Usage :
    python .claude/skills/rag-concepts/scripts/concepts_lot.py <document_id | pdf | dossier> [--batch-size 40] [--reset]

`--reset` (a n'utiliser que sur le tout premier lot) est transmis tel quel a
/rag-concepts. Sans lui, la commande reprend le lot suivant.
...
Sortie : la sortie brute complete de /rag-concepts (stdout et stderr
fusionnes, en UTF-8), puis UNE ligne de bilan commencant par [LOT n/N], [FIN],
[BLOQUANT] ou [ERREUR].

Code de sortie DE CE SCRIPT (celui de /rag-concepts, lui, reste 3 sur un lot
partiel -- convention du pipeline, inchangee dans run.py) :
    ...
    2  BLOQUANT ([BLOQUANT]) : > 5 % de chunks sans concepts, aucun concept...
"""
...
def summarize(out: str, returncode: int, batch_size: int, seconds: float) -> tuple[str, int]:
    ...
    /rag-concepts et de son code de retour. Separee de main() pour etre testable."""
    ...
    if returncode == 3:
        ...
        # chunk(s) traite(s) (S reussi(s)), R restant(s)"), pas celui de la
        # ligne de debut de lot ("Lot en cours : N chunk(s) sur T restant(s)").
        final = re.search(r"(\d+) restant\(s\)", out)
    ...
    if returncode == 0:
        ...
        detail = f"{ok.group(1)} mention(s) de concept" if ok else "termine"
        state = "deja fait" if "deja fait" in out else "termine"
        return f"[FIN] /rag-concepts {state} en {duration} : {detail} ({done}/{total} chunks).", 0
    if returncode == 2:
        return f"[BLOQUANT] /rag-concepts arrete en {duration} apres {done}/{total} chunks -- voir le detail ci-dessus. NE PAS enchainer.", 2
    return f"[ERREUR] /rag-concepts a echoue (code {returncode}) en {duration} -- voir le detail ci-dessus.", 1
...
def main() -> int:
    ...
    parser.add_argument("--reset", action="store_true", help="premier lot seulement : regenere entierement concepts.json")
